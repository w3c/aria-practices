...
const checkFocus = async function (t, selector) {
  return await t.context.session.wait(async function () {
    return t.context.session.executeScript(function () {
      const [ selector ] = arguments;
      const items = document.querySelector(selector);
      return items === document.activeElement;
    }, selector);
  }, t.context.waitTime, 'Timeout waiting for dom focus on element:' + selector);
};
...
ariaTest('"aria-haspopup" on button element', exampleFile, 'button-aria-haspopup', async (t) => {
    await assertAttributeValues(t, ex.buttonSelector, 'aria-haspopup', 'listbox');
});
...
ariaTest('Character keys moves focus', exampleFile, 'key-character', async (t) => {
  ...
  await assertAriaSelectedAndActivedescendant(t, ex.listboxSelector, ex.optionSelector, 5);
  ...
  await assertAriaSelectedAndActivedescendant(t, ex.listboxSelector, ex.optionSelector, 4);
  ...
  await listbox.sendKeys('b');
  await assertAriaSelectedAndActivedescendant(t, ex.listboxSelector, ex.optionSelector, 14);
});
