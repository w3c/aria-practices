...
ariaTest('key ENTER expands details', exampleFile, 'key-enter-or-space', async (t) => {
  ...
  for (let index = 0; index < ex.buttonSelectors.length; index++) {
    ...
    t.true(
      await t.context.session.findElement(By.css(answerSelector)).isDisplayed(),
      'Answer should be displayed after sending ENTER to button: ' + buttonSelector
    );
    ...
    t.true(
      await t.context.session.findElement(By.css(answerSelector)).isDisplayed(),
      'Answer should not be displayed after sending ENTER twice to button: ' + buttonSelector
    );
  }
});
...
ariaTest('key SPACE expands details', exampleFile, 'key-enter-or-space', async (t) => {
  ...
  for (let index = 0; index < ex.buttonSelectors.length; index++) {
    ...
    t.true(
      await t.context.session.findElement(By.css(answerSelector)).isDisplayed(),
      'Answer should be displayed after sending SPACE to button: ' + buttonSelector
    );
    ...
    t.true(
      await t.context.session.findElement(By.css(answerSelector)).isDisplayed(),
      'Answer should not be displayed after sending SPACE twice to button: ' + buttonSelector
    );
  }
});
