...
const exampleFile = 'radio/radio-fan.html';

const ex = {
  radiogroupSelector: '#ex1 [role="radiogroup"]',
  radioSelector: '#ex1 [role="radio"]',
  radiogroupSelectors: ['#ex1 [role="radiogroup"]:nth-of-type(1)'],
  radioSelectors: ['#ex1 [role="radiogroup"]:nth-of-type(1) [role="radio"]'],
...
ariaTest('role="radio" on g elements', exampleFile, 'radio-role', async (t) => {
  await assertAriaRoles(t, 'ex1', 'radio', '4', 'g');
});
...
ariaTest(
  ...
  async (t) => {
    ...
    await assertTabOrder(t, [ex.radiogroupSelectors[0]]);
...
ariaTest('Selects radio item', exampleFile, 'key-space', async (t) => {
  await t.context.session
    ...
    .sendKeys(' ');
  const firstRadioOption = ex.radioSelectors[0] + ':nth-of-type(1)';
  await assertAttributeValues(t, firstRadioOption, 'aria-checked', 'true');
});
...
ariaTest(
  ...
  async (t) => {
    for (
      let groupIndex = 0;
      groupIndex < ex.radiogroupSelectors.length;
      groupIndex++
    ) {
