...
const checkVisuallyChecked = async function (t, selector, checked) {
  let expectedBackgroundColor = checked ? 'rgb(96, 155, 251)' : 'rgba(0, 0, 0, 0)';
...
ariaTest('role="checkbox" elements exist', exampleFile, 'checkbox-role', async (t) => {
  ...
  for (let index = 0; index < checkboxes.length; index++) {
    ...
    t.true(
      ...
      'checkbox div at index: ' + index + ' should have contain text describing the textbox'
    );
  }
});
...
ariaTest('"aria-checked" on checkbox element', exampleFile, 'checkbox-aria-checked', async (t) => {
  ...
  await assertAttributeValues(t, ex.checkboxSelector, 'aria-checked', 'false');
...
ariaTest('key SPACE selects or unselects checkbox', exampleFile, 'key-space', async (t) => {
  ...
  for (let checkboxSelector of ex.checkboxes) {
    ...
    await t.context.session.findElement(By.css(checkboxSelector)).sendKeys(Key.SPACE);

    t.true(
      await waitAndCheckAriaChecked(t, checkboxSelector, 'true'),
      'aria-selected should be set after sending SPACE key to checkbox: ' + checkboxSelector
    );

    t.true(
      await checkVisuallyChecked(t, checkboxSelector, true),
      'checkbox should be visual checked after sending SPACE key to checkbox: ' + checkboxSelector
    );
