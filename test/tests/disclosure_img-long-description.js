...
ariaTest('TAB should move focus to button', exampleFile, 'key-tab', async (t) => {
  ...
  t.true(
    focusIsOnButton,
    'The disclosure button (' + ex.buttonSelector + ') should be reachable in tab sequence.'
  );
});
...
ariaTest('key ENTER expands details', exampleFile, 'key-space-or-enter', async (t) => {
  ...
  t.is(
    ...
    'true',
    'Button should have aria-expanded true after sending ENTER to "' + ex.buttonSelector + '"'
  );

  t.true(
    await description.isDisplayed(),
    'Description should be displayed after sending ENTER to button to "' + ex.buttonSelector + '"'
  );
  ...
  t.is(
    await button.getAttribute('aria-expanded'),
    'false',
    'Button should have aria-expanded false after sending ENTER twice to "' + ex.buttonSelector + '"'
  );

  t.false(
    await description.isDisplayed(),
    'Description should not be displayed after sending ENTER twice to button to "' + ex.buttonSelector + '"'
  );
});
...
ariaTest('key SPACE expands details', exampleFile, 'key-space-or-enter', async (t) => {
  ...
  t.is(
    await button.getAttribute('aria-expanded'),
    'true',
    'Button should have aria-expanded true after sending SPACE to "' + ex.buttonSelector + '"'
  );

  t.true(
    await description.isDisplayed(),
    'Description should be displayed after sending SPACE to button to "' + ex.buttonSelector + '"'
  );
  ...
  t.is(
    await button.getAttribute('aria-expanded'),
    'false',
    'Button should have aria-expanded false after sending SPACE twice to "' + ex.buttonSelector + '"'
  );

  t.false(
    await description.isDisplayed(),
    'Description should not be displayed after sending SPACE twice to button to "' + ex.buttonSelector + '"'
  );
});
