...
const exampleFile = 'carousel/carousel-2/carousel-2.html';
...
ariaTest('Carousel 2: pause button has aria-pressed', exampleFile, 'pause-aria-pressed', async (t) => {
  t.plan(1);
  await assertAttributeValues(t, ex.pauseSelector, 'aria-pressed', 'false');
});

ariaTest('Carousel 2: section has aria-label', exampleFile, 'landmark-aria-label', async (t) => {
  t.plan(1);
