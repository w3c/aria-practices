...
const ex = {
  ...
  firstPanelInputSelectors: ['#cufc1', '#cufc2', '#cufc3', '#cufc4', '#cufc5', '#cufc6'],
  secondPanelInputSelectors: ['#b-add1', '#b-add2', '#b-city', '#b-state', '#b-zip'],
  thirdPanelInputSelectors: ['#m-add1', '#m-add2', '#m-city', '#m-state', '#m-zip']
};
...
ariaTest('h3 element should wrap accordion button', exampleFile, 'h3-element', async (t) => {
  ...
  for (let button of buttons) {
    t.is(
      ...
      'H3',
      'Parent of button ' + (await button.getText()) + 'should be an H3 element'
    );
  }
});
  ...
  for (let expandIndex = 0; expandIndex < buttons.length; expandIndex++) {
    ...
    for (let index = 0; index < buttons.length; index++) {
      ...
      t.is(
        ...
        expandedValue,
        'Accordion button at index ' + expandIndex + ' has been clicked, therefore ' +
          '"aria-expanded" on button ' + index + ' should be "' + expandedValue
      );
    }
  }
});
  ...
  for (let expandIndex = 0; expandIndex < buttons.length; expandIndex++) {
    ...
    for (let index = 0; index < buttons.length; index++) {
      ...
      t.is(
        ...
        disabledValue,
        'Accordion button at index ' + expandIndex + ' has been clicked, therefore ' +
          '"aria-disabled" on button ' + index + ' should be "' + disabledValue
      );
    }
  }
});
...
ariaTest('role "region" exists on accordion panels', exampleFile, 'region-role', async (t) => {
  ...
  for (let panelId of panelIds) {
    t.is(
      await t.context.session.findElement(By.id(panelId)).getAttribute('role'),
      'region',
...
ariaTest('ENTER key expands section', exampleFile, 'key-enter-or-space', async (t) => {
  ...
  for (let expandIndex of [1, 2, 0]) {
    ...
    t.true(
      ...
      'Sending key ENTER to button at index ' + expandIndex + ' should expand the region.'
    );

    t.is(
      await buttons[expandIndex].getAttribute('aria-expanded'),
      'true',
      'Sending key ENTER to button at index ' + expandIndex + ' set aria-expanded to "true".'
    );

    t.is(
      await buttons[expandIndex].getAttribute('aria-disabled'),
      'true',
      'Sending key ENTER to button at index ' + expandIndex + ' set aria-disable to "true".'
    );
    ...
    t.true(
      await panels[expandIndex].isDisplayed(),
      'Sending key ENTER twice to button at index ' + expandIndex + ' do nothing.'
    );
  }
});
...
ariaTest('SPACE key expands section', exampleFile, 'key-enter-or-space', async (t) => {
  ...
  for (let expandIndex of [1, 2, 0]) {
    ...
    t.true(
      await panels[expandIndex].isDisplayed(),
      'Sending key SPACE to button at index ' + expandIndex + ' should expand the region.'
    );

    t.is(
      await buttons[expandIndex].getAttribute('aria-expanded'),
      'true',
      'Sending key SPACE to button at index ' + expandIndex + ' set aria-expanded to "true".'
    );

    t.is(
      await buttons[expandIndex].getAttribute('aria-disabled'),
      'true',
      'Sending key SPACE to button at index ' + expandIndex + ' set aria-disable to "true".'
    );
    ...
    t.true(
      await panels[expandIndex].isDisplayed(),
      'Sending key SPACE twice to button at index ' + expandIndex + ' do nothing.'
    );
  }
});
...
ariaTest('SHIFT+TAB moves focus between headers and displayed inputs', exampleFile, 'key-shift-tab', async (t) => {
  ...
  await t.context.session.findElement(By.css(lastElement)).sendKeys(Key.chord(Key.SHIFT, Key.TAB));
  ...
  for (let index = elementsInOrder.length - 1; index >= 0; index--) {
    ...
    await t.context.session.findElement(By.css(itemSelector)).sendKeys(Key.chord(Key.SHIFT, Key.TAB));
  }
...
ariaTest('DOWN ARROW moves focus between headers', exampleFile, 'key-down-arrow', async (t) => {
  ...
  for (let index = 0; index < ex.buttonsInOrder.length - 1; index++) {
    ...
    await t.context.session.findElement(By.css(itemSelector)).sendKeys(Key.ARROW_DOWN);
...
ariaTest('UP ARROW moves focus between headers', exampleFile, 'key-up-arrow', async (t) => {
  ...
  for (let index = ex.buttonsInOrder.length - 1; index > 0; index--) {
    ...
    let itemSelector = ex.buttonsInOrder[index];
    await t.context.session.findElement(By.css(itemSelector)).sendKeys(Key.ARROW_UP);
  ...
  t.true(
    await focusMatchesElement(t, ex.buttonsInOrder[ex.buttonsInOrder.length - 1]),
    'Focus should reach element: ' + ex.buttonsInOrder[ex.buttonsInOrder.length - 1]
  );
...
ariaTest('HOME key will always move focus to first button', exampleFile, 'key-home', async (t) => {
  ...
  for (let index = 0; index <= lastIndex; index++) {
    ...
    let itemSelector = ex.buttonsInOrder[index];
    await t.context.session.findElement(By.css(itemSelector)).sendKeys(Key.HOME);
...
ariaTest('END key will always move focus to last button', exampleFile, 'key-end', async (t) => {
  ...
  for (let index = lastIndex; index >= 0; index--) {
    ...
    let itemSelector = ex.buttonsInOrder[index];
    await t.context.session.findElement(By.css(itemSelector)).sendKeys(Key.END);
