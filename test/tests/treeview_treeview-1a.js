...
const exampleFile = 'content/patterns/treeview/examples/treeview-1a.html';
    ...
    while (i < items.length) {
      ...
      // If the folder is an open folder, the focus will move
        ...
        t.true(
          ...
          'Sending key ARROW_RIGHT to open folder at treeitem index ' +
            i +
...
ariaTest('key home moves focus', exampleFile, 'key-home', async (t) => {
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    t.true(
      ...
      'Sending key HOME to folder/item at index ' +
        i +
...
ariaTest('key end moves focus', exampleFile, 'key-end', async (t) => {
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    t.true(
      ...
      'Sending key END to folder/item at index ' +
        i +
